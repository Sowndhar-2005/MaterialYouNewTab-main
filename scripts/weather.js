...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function applyWeatherState(hidden) {
        ...
        if (hideWeather) hideWeather.classList.toggle("weather-hidden", hidden);
    }
...
async function getWeatherData() {
    ...
    // Display texts 
    if (conditionTextEl) conditionTextEl.textContent = translations[currentLanguage]?.conditionText || translations["en"].conditionText;
    if (humidityLevelEl) humidityLevelEl.textContent = translations[currentLanguage]?.humidityLevel || translations["en"].humidityLevel;
    if (locationEl) locationEl.textContent = translations[currentLanguage]?.location || translations["en"].location;
    ...
    if (userLocInput && savedLocation) userLocInput.value = savedLocation;
    if (userAPIInput && savedApiKey) userAPIInput.value = savedApiKey;
    ...
    if (minMaxTempCheckbox) minMaxTempCheckbox.checked = isMinMaxEnabled;

    const feelsLikeElement = document.getElementById("feelsLike");
    if (feelsLikeElement) {
        feelsLikeElement.textContent = isMinMaxEnabled
    ...
    if (userAPIInput) userAPIInput.addEventListener("keydown", (event) => handleEnterPress(event, "saveAPI"));
    if (userLocInput) userLocInput.addEventListener("keydown", (event) => handleEnterPress(event, "saveLoc"));
    ...
    if (gpsToggle) {
        gpsToggle.addEventListener("change", async () => {
            if (gpsToggle.checked) {
                ...
                if (locationCont) locationCont.classList.add("inactive");
            } else {
                localStorage.setItem("useGPS", false);
                if (locationCont) locationCont.classList.remove("inactive");
            }
            location.reload();
        });
    }
    ...
    if (saveLocButton) {
        saveLocButton.addEventListener("click", () => {
            const userLocation = userLocInput ? userLocInput.value.trim() : "";
            if (userLocation) localStorage.setItem("weatherLocation", userLocation);
            localStorage.setItem("useGPS", false);
    ...
    if (userLocInput) {
        ...
        userLocInput.addEventListener("keydown", (e) => {
            const items = locationSuggestions ? locationSuggestions.querySelectorAll(".location-suggestion-item") : [];
    ...
    async function fetchWeather() {
        try {
            ...
            function UpdateWeather() {
                ...
                const conditionTextEl = document.getElementById("conditionText");
                if (conditionTextEl) conditionTextEl.textContent = conditionText;
                ...
                const humidityLevelEl = document.getElementById("humidityLevel");
                if (humidityLevelEl) {
                    humidityLevelEl.textContent = isRTL
                ...
                const updateTemperatureDisplay = () => {
                    ...
                    if (isFahrenheit) {
                        // Update temperature
                        if (tempElement) tempElement.textContent = localizedTempFahrenheit;
                        const tempUnitF = document.createElement("span");
                        tempUnitF.className = "tempUnit";
                        tempUnitF.textContent = "°F";
                        if (tempElement) tempElement.appendChild(tempUnitF);
                        ...
                        // Update temperature
                        if (tempElement) tempElement.textContent = localizedTempCelsius;
                        const tempUnitC = document.createElement("span");
                        tempUnitC.className = "tempUnit";
                        tempUnitC.textContent = "°C";
                        if (tempElement) tempElement.appendChild(tempUnitC);
                ...
                const locationIcon = locationTile ? locationTile.querySelector(".location-icon") : null;
